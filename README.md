...
The `useRouteMatch` hook attempts to match the current URL in the same way that a `<Route>` would. It’s mostly useful for getting access to the match data without actually rendering a `<Route>`. -reactrouter.com

2. How would you explain form validation to someone who has never programmed before?

So far what I’ve learned is that form validation can be used to warn and instruct the user as they are filling out the form to do it the right way. This saves them the trouble of getting to the end of the form and clicking submit only to find that they have to go back and correct what they’ve done or missed before they can try submitting again. Also we can set a rule that does not allow the submit button to be enabled until all the form validation rules are met. Required fields must be filled in with the proper type and format of information. Along with form validation are good placeholders so it’s clear what the end user should do with each field. Also required fields should be marked as such.

3. In 1-2 sentences, define end to end testing.

End to end testing simulates a user’s “happy path” so each step can be tested along the way with all reasonable possible variations. We want to see the form validation succeed and we want to see the actions we expect to happen, happen, with a final, expected result. 
